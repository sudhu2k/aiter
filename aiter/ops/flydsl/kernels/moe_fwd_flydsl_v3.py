...
This is a straight port of the MegaMOE grouped bf16 GEMM (``primus_turbo.flydsl`` --
32x32x16 MFMA, 8-wave / 512-thread workgroup, deep distance-2 3-buffer DMA ring, XCD swizzle
with ``GROUP_M`` front-loading) into aiter, with a *single* change: the A operand is fetched
...
def _bootstrap_primus_turbo() -> None:
    """Put the MegaMOE (primus-turbo) package on ``sys.path``.

    v3 reuses MegaMOE's compiled GEMM primitives verbatim so codegen -- and therefore
    performance -- is identical to the Mega baseline. Override the location with
    ``PRIMUS_TURBO_PATH`` if it is not at the default checkout.
    """
    try:
        ...
        pass
    cand = os.environ.get("PRIMUS_TURBO_PATH", "/tmp/primus-turbo-full")
    if cand and cand not in sys.path and os.path.isdir(cand):
        sys.path.insert(0, cand)


_bootstrap_primus_turbo()
